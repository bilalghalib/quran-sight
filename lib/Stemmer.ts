...
export default class Stemmer {
    affixCleaner: AffixCleaner | null;

    constructor() {
        this.affixCleaner = null;
    }

    stem(token: string): StemResult | string {
        token = token.trim();
        ...
        matches = matches.reduce((res: string[], current: string) => {
    ...
    getMatches(token: string, removeFirst: 'suffix' | 'prefix' = 'suffix', inRecursion: boolean = false): string[] {
    ...
    getMatchesForPatterns(token: string, patterns: RegExp[]): string[] {
        const matches: string[] = [];
        for (const pat of patterns) {
            const match = pat.exec(token);
            if (match) {
    ...
    preNormalize(token: string): string {
    ...
    postNormalize(token: string): string {
        if (token.length === 3) {
